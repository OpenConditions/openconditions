...
describe("existing-feed regression — pre-commons observation", () => {
  it("lands a plain observation with default fuzziness/privacy_class and NULL elsewhere", async () => {
    ...
    expect(r.privacy_class).toBe("unknown");
    expect(r.instance_id).toBeNull();
    ...
    expect(r.source_license).toBeNull();
  }, 30_000);
});
