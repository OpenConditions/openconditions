...
/** Project using this service's explicit domain-attribute dispatch. */
export function toRow(obs: Observation) {
  return projectObservationRow(obs, DOMAIN_REGISTRY[obs.domain]?.attributes(obs) ?? {});
}
...
export async function upsertRows(
  ...
  const touched = await tx<{ id: string; inserted: boolean }[]>`
    ...
    WHERE conditions.observations.content_hash IS DISTINCT FROM excluded.content_hash
       OR conditions.observations.origin IS DISTINCT FROM excluded.origin
       OR conditions.observations.source_format IS DISTINCT FROM excluded.source_format
       OR conditions.observations.instance_id IS DISTINCT FROM excluded.instance_id
       OR conditions.observations.canonical_id IS DISTINCT FROM excluded.canonical_id
       OR conditions.observations.privacy_class IS DISTINCT FROM excluded.privacy_class
