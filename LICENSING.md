...
| The federation service — instance actor document and peers discovery (`services/federation`)                                                                                                                                                                                 | **AGPL-3.0-or-later** | Same rationale as the ingest service: a deployable network server whose hosted derivatives must stay open.                                                                                                                                          |
| Shared persistence projection (`packages/storage`) and isolated experimental probe (`packages/probe-spike`)                                                                                                                                                                  | **AGPL-3.0-or-later** | The persistence projection retains its original ingest-service license; the probe remains an isolated AGPL experiment.                                                                                                                              |
| Reusable libraries ( `@openconditions/core`, `roads`, `publishers`, `openlr`, `federation`, `contrib-core`, `normalize`), the OpenMapX provider integration (`integrations/road-conditions-openconditions`), and the standalone OpenLR resolver (`services/openlr-resolver`) | **Apache-2.0**        | These have value outside OpenConditions — the canonical model, the parsers/emitters, the write-normalization seam, and a generic OpenLR map-matcher. A permissive license (with an explicit patent grant) lets any project depend on or embed them. |
