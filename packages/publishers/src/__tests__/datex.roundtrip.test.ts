 ...
 * (Simple-enum elements like road name / management type are emitted as standard
 * text; our ingest reader only extracts object-shaped children, so those are not
 * asserted here — a reader limitation, not an emitter defect.)
 */
...
describe("DATEX II round-trip through the ingest parser", () => {
  ...
  it("re-ingests a closure as a road-management event (DATEX models closures as management records)", () => {
    const xml = observationsToDatexSituations([
      roadEvent({ id: "ndw:rt2", type: "road_closure", roadState: "closed" }),
    ]);
    const [ev] = parseDatexSituations(xml, SRC);
    expect(ev).toBeDefined();
    expect(ev!.type).toBe("lane_closure");
  });
});
