...
describe("createMeasuredDataParser — DATEX v1 shape (element-text ref, direct-text speed, point site)", () => {
  // Some DATEX v1 MeasuredData feeds carry the site id as element TEXT (not an
  // @id attribute) and the speed as the direct text of <averageVehicleSpeed>
  // (not a nested <speed>), with the companion site table placing each site by a
  // bare pointCoordinates Point. The Norway measured feed relies on this path.
  const V1_SITES = `<?xml version="1.0" encoding="UTF-8"?>
<d2LogicalModel><payloadPublication xsi:type="MeasurementSiteTablePublication">
  ...
  const V1_DATA = `<?xml version="1.0" encoding="UTF-8"?>
<d2LogicalModel><payloadPublication xsi:type="MeasuredDataPublication">
  ...
  const V1_SOURCE = {
    id: "datex-v1-flow",
    attribution: "DATEX v1 example",
    country: "NO",
    license: "NLOD-2.0",
  } as const;

  it("resolves the element-text site id against a point site table and reads direct-text speed", () => {
    const siteMap = parseDatexSiteTable(V1_SITES);
    expect(siteMap.get("S601")).toEqual({ type: "Point", coordinates: [8.468946, 60.472024] });

    const parser = createMeasuredDataParser(V1_SOURCE, siteMap, () => "2026-06-24T10:10:00.000Z");
    parser.write(V1_DATA);
    const out = parser.close();

    expect(out.failed).toBeFalsy();
    expect(out.flows).toHaveLength(1);
    expect(out.flows[0]!.id).toBe("datex-v1-flow:S601");
    expect(out.flows[0]!.speedKph).toBe(101);
    expect(out.flows[0]!.geometry).toEqual({ type: "Point", coordinates: [8.468946, 60.472024] });
  });
});
